...
// single set of LED(s) with 2 different power channels, regulated buck + FET.
// only buck, or only FET is active at any point.
void set_level_udrf(uint8_t level) {
...
bool gradual_tick_main(uint8_t gt) {
    ...
    // gt, or gradual-target, is the target anduril ramp level 
